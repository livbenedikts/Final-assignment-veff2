...
const Selectusername = () => {
    ...
    return (
        <div className='usernameContainer'>
            ...
            autoComplete="off"
            >
            <TextField id="username-text-field" value={username} onChange={(event) => {setUsername(event.target.value)}} label="username" variant="standard" />
            <Button onClick={addUser(username)} variant="contained" disabled={!username}>Join Chat</Button>
            </Stack>
            {/* {!isusernameAvailable && <p>{failedMessage}</p>} */}
        </div>
    );
